...
# Make sure to download the punkt tokenizer models before running this function
# nltk.download('punkt')

def count_prompt_and_sampled_tokens(conversation):
    prompt_token_count = 0
    sampled_token_count = 0
    # ... existing code to retrieve conversation parts ...
    for part in conversation_parts:
        # Use NLTK's word_tokenize instead of split
        prompt_token_count += len(word_tokenize(part.strip()))
        sampled_token_count += len(word_tokenize(part.strip()))
